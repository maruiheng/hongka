...
/**
 * 3.2.1.7响应查询Tracker静默时段(R_Q_FORBID_RANGE)
 *
 * @author Allen
 */
public class TrackerForbidRangeQueryRes extends Packet {
    public static final String CMD = "R_Q_FORBID_RANGE";
    private List<TrackerMuteResp> data;
    private String msg;

    public TrackerForbidRangeQueryRes() {
        super(CMD);
    }
    ...
    public void decodeArgs(String[] sa, int offset, int length) {
        ...
        try {
            if (status.equals(NettyClientManager.STR_CODE_OK)) {
                ...
                msg = jsonData;
                Gson gson = new Gson();
                data = gson.fromJson(msg, new TypeToken<List<TrackerMuteResp>>() {
                }.getType());
            } else {
                msg = sa[offset++];
            }
